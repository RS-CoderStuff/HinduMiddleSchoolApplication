...
class Batch_Form(forms.ModelForm):
    ...
    teachers = forms.ModelMultipleChoiceField(required=True,queryset=User.objects.filter(user_type=1),widget=autocomplete.ModelSelect2Multiple(url='teacher-autocomplete',attrs={'class': "inputfield"}))
    ...
    class Meta:
        model = Batch_Db
        fields = ['name', 'description', 'start_time', 'end_time', 'category', 'students','teachers',]
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk != None:
            ...
            print(aa,'pppppppppppppppppppppp')

            # print(self.instance.pk,abc.id,'hhhhhhhhhhhhhhhhhhhhhh')
            self.fields['students'] = forms.ModelMultipleChoiceField(required=True, queryset=User.objects.filter(id__in=aa,user_type=2),
                                                       widget=autocomplete.ModelSelect2Multiple(
                                                          url='get_main_course_user', attrs={'class': "inputfield"}))

        else:
            self.fields['students']  = forms.ModelMultipleChoiceField(required=True, queryset=User.objects.filter(user_type=2),
                                                      widget=autocomplete.ModelSelect2Multiple(
...
class student_form(forms.Form):
    ...
    students = forms.ModelMultipleChoiceField(required=True,queryset=User.objects.filter(user_type=2),widget=autocomplete.ModelSelect2Multiple(url='get_main_course_user',attrs={'class': "inputfield"}))
